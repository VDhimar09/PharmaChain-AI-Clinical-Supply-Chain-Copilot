...
class DocumentService:
    ...
    def __init__(
        ...
        self.storage = storage or (
            LocalDocumentStorage(storage_dir)
            if storage_dir is not None
            else get_document_storage()
        )
        self.storage_backend = "local" if isinstance(self.storage, LocalDocumentStorage) else "s3"
    ...
    def upload_document(
        ...
        *,
        ...
        # This key contains only UUIDs generated by the server. It is portable
        # across backends and never incorporates the uploaded filename.
        storage_key = f"{document_id}-{uuid.uuid4()}.pdf"
        stored_filename = self.storage.store(file_bytes, storage_key, mime_type)
        ...
        with self.storage.materialize(document.storage_key) as file_path:
    ...
    def delete_document(self, document_id: uuid.UUID) -> None:
        ...
        try:
            self._storage_for_document(document).delete(document.storage_key)
    ...
    def create_download_url(self, document: Document) -> str | None:
        return self._storage_for_document(document).create_download_url(
            document.storage_key,
            document.original_filename,
            settings.DOCUMENT_S3_PRESIGNED_URL_EXPIRES_SECONDS,
        )

    def materialize_document(self, document: Document):
        return self._storage_for_document(document).materialize(document.storage_key)

    def _storage_for_document(self, document: Document) -> DocumentStorage:
        if document.storage_backend == self.storage_backend:
            return self.storage
        return get_document_storage(document.storage_backend)
